...
if [ "$SERVICE" = "task-service" ] || [ "$SERVICE" = "all" ]; then
    APP_LOG_LEVEL="INFO" APP_HOST="$TASK_APP_HOST" APP_PORT="$TASK_APP_PORT" TASK_DB_HOST="$TASK_DB_HOST" TASK_DB_PORT="$TASK_DB_PORT" TASK_DB_USER="$TASK_DB_USER" TASK_DB_PASSWORD="$TASK_DB_PASSWORD" TASK_DB_NAME="$TASK_DB_NAME" ./task-service task
fi

if [ "$SERVICE" = "auth-service" ] || [ "$SERVICE" = "all" ]; then
    APP_LOG_LEVEL="INFO" APP_HOST="$AUTH_APP_HOST" APP_PORT="$AUTH_APP_PORT" AUTH_DB_HOST="$AUTH_DB_HOST" AUTH_DB_PORT="$AUTH_DB_PORT" AUTH_DB_USER="$AUTH_DB_USER" AUTH_DB_PASSWORD="$AUTH_DB_PASSWORD" AUTH_DB_NAME="$AUTH_DB_NAME" AUTH_JWT_SECRET="random secret" ./auth-service authn
fi
